...
GatewaySocket::GatewaySocket(boost::asio::io_service &service, std::function<void (Socket *)> closeHandler)
...
void GatewaySocket::SendPacket(const GatewayPacket& pct, bool immediate)
{
    if (IsClosed())
        return;

    // Dump outgoing packet.
    sLog.outGatewayPacketDump(GetRemoteEndpoint().c_str(), pct.GetOpcode(), pct.GetOpcodeName(), &pct, false);
...
bool GatewaySocket::Open()
{
    if (!Socket::Open())
        return false;

    // Send startup packet.
    GatewayPacket packet(SMSG_AUTH_CHALLENGE, 40);
    packet << m_seed;
...
bool GatewaySocket::ProcessIncomingData()
{
	...
	if ((header.size < 4) || (header.size > MAX_SOCKET_BUF) || (header.cmd >= NUM_MSG_TYPES))
    {
        sLog.outError("GatewaySocket::ProcessIncomingData: client sent malformed packet size = %u , cmd = %u", header.size, header.cmd);
    ...
    GatewayPacket *pct = new GatewayPacket(opcode, validBytesRemaining);
    ...
    sLog.outGatewayPacketDump(GetRemoteEndpoint().c_str(), pct->GetOpcode(), pct->GetOpcodeName(), pct, true);
    ...
    try
    {
        switch (opcode)
        {
            case CMSG_AUTH_SESSION:
                if (m_session)
                {
                    sLog.outError("GatewaySocket::ProcessIncomingData: Player send CMSG_AUTH_SESSION again");
                    return false;
                }
            ...
            default:
            {
                if (!m_session)
                {
                    sLog.outError("GatewaySocket::ProcessIncomingData: Client not authed opcode = %u", uint32(opcode));
                    return false;
                }
    ...
    {
        sLog.outError("GatewaySocket::ProcessIncomingData ByteBufferException occured while parsing an instant handled packet (opcode: %u) from client %s, accountid=%i.",
...
bool GatewaySocket::HandleAuthSession(GatewayPacket &recvPacket)
{
    ...
    Sha1Hash sha1;
    BigNumber v, s, g, N, K;
    GatewayPacket packet, SendAddonPacked;
    ...
    DEBUG_LOG("GatewaySocket::HandleAuthSession: client build %u, account %s, clientseed %X",
              ClientBuild,
              account.c_str(),
              clientSeed);
    ...
    if (!IsAcceptableClientBuild(ClientBuild))
    {
        ...
        sLog.outError("GatewaySocket::HandleAuthSession: Sent Auth Response (version mismatch).");
        return false;
    }
    ...
    if (!result)
    {
        ...
        sLog.outError("GatewaySocket::HandleAuthSession: Sent Auth Response (unknown account).");
        return false;
    }
    ...
    DEBUG_LOG("GatewaySocket::HandleAuthSession: (s,v) check s: %s v: %s",
              sStr,
              vStr);
    ...
    if (fields[4].GetUInt8() == 1)  // if ip is locked
    {
        if (strcmp(fields[3].GetString(), GetRemoteAddress().c_str()))
        {
            ...
            delete result;
            BASIC_LOG("GatewaySocket::HandleAuthSession: Sent Auth Response (Account IP differs).");
            return false;
        }
    }
    ...
    if (banresult) // if account banned
    {
        ...
        sLog.outError("GatewaySocket::HandleAuthSession: Sent Auth Response (Account banned).");
        return false;
    }

    // Check locked state for server
    AccountTypes allowedAccountType = sGateway.GetPlayerSecurityLimit();
    ...
    if (allowedAccountType > SEC_PLAYER && AccountTypes(security) < allowedAccountType)
    {
        ...
        BASIC_LOG("GatewaySocket::HandleAuthSession: User tries to login but his security level is not enough");
        return false;
    }
    ...
    if (memcmp(sha.GetDigest(), digest, 20))
    {
        ...
        sLog.outError("GatewaySocket::HandleAuthSession: Sent Auth Response (authentification failed).");
        return false;
    }

    const std::string &address = GetRemoteAddress();

    DEBUG_LOG("GatewaySocket::HandleAuthSession: Client '%s' authenticated successfully from %s.",
              account.c_str(),
              address.c_str());
    ...
    if (!(m_session = new GatewaySession(id, this, AccountTypes(security), mutetime, locale)))
        return false;
...
bool GatewaySocket::HandlePing(GatewayPacket &recvPacket)
{
    uint32 ping;
    uint32 latency;
    ...
    else
    {
        ...
        if (seconds.count() < 27)
        {
            ...
            const uint32 max_count = sGateway.getConfig(CONFIG_UINT32_MAX_OVERSPEED_PINGS);

            if (max_count && m_overSpeedPings > max_count)
            {
                if (m_session && m_session->GetSecurity() == SEC_PLAYER)
                {
                    sLog.outError("GatewaySocket::HandlePing: Player kicked for "
    ...
    // critical section
    {
        ...
        {
            sLog.outError("GatewaySocket::HandlePing: peer sent CMSG_PING, "
    ...
    GatewayPacket packet(SMSG_PONG, 4);
    packet << ping;
