...
#ifndef __LOGICSESSION_H
#define __LOGICSESSION_H
...
/// Player session in the Logic
class MANGOS_DLL_SPEC LogicSession : public BaseSession
{
	friend class CharacterHandler;

public:
	LogicSession(LogicSocket* sock, ServerNodeType sec, uint32 node_index);
	~LogicSession();
	...
	*/
	...
	//      void Handle_EarlyProccess(WorldPacket& recvPacket); // just mark packets processed in LogicSocket::OnRead
	...
	//      void HandleGatewayTeleportOpcode(WorldPacket& recv_data);
...
private:
	...
	void ExecuteOpcode(OpcodeHandler_g const& opHandle, WorldPacket* packet);
	...
	Player * _player;
	LogicSocket * const m_Socket;                       // socket pointer is owned by the network thread which created 
