...
/// Update the DBServerSession (triggered by DBServer update)
bool DBServerSession::Update(PacketFilter& updater)
{
	...
	while (m_Socket && !m_Socket->IsClosed() && !m_recvQueue.empty())
	{
		...
		{
			sLog.outError("DBServerSession::Update ByteBufferException occured while parsing a packet (opcode: %u) from client %s, accountid=%i.",
	...
	// logout procedure should happen only in DBServer::UpdateSessions() method!!!
...
void DBServerSession::LogoutPlayer(bool save)
{
	...
	//		// this is a bad place to call for far teleport because we need player to be in DBServer for successful logout
	...
	//	// no point resetting online in character table here as Player::SaveToDB() will set it to 1 since player has not been removed from DBServer at this stage
	...
	//	///- Remove the player from the DBServer
	//	// the player may not be in the DBServer when logging out
	...
	//	SetPlayer(nullptr);                                    // deleted in Remove/DeleteFromDBServer call
...
/// Kick a player out of the DBServer
...
void DBServerSession::LoadTutorialsData()
{
...
void DBServerSession::SendTutorialsData()
{
...
void DBServerSession::SaveTutorialsData()
{
...
void DBServerSession::ExecuteOpcode(OpcodeHandler_g const& opHandle, WorldPacket* packet)
{
