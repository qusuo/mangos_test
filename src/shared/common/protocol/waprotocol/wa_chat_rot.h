...
namespace waprot
{
	...
	{
		WAReqBroadcast()
			:header(CHAT_REQ_BROADCAST_WA, SERVER_NODE_TYPE_GLOBAL, prot::SS_MSG_TYPE_SYS)
			, broadcast_level(0)
			, role_id(0)
			, res_code(0)
		{
	...
	struct WAChatAllianceCreate
	{
		WAChatAllianceCreate()
			:header(CHAT_ALLIANCE_CREATE_WA, SERVER_NODE_TYPE_GLOBAL, prot::SS_MSG_TYPE_SYS)
			, alliance_id(0)
		{
		}
		prot::SSHeader header;
	...
	struct WAChatAllianceDestroy
	{
		WAChatAllianceDestroy()
			:header(CHAT_ALLIANCE_DESTROY_WA, SERVER_NODE_TYPE_GLOBAL, prot::SS_MSG_TYPE_SYS)
			, alliance_id(0)
		{
		}
		prot::SSHeader header;
	...
	struct WAChatRoleInAlliance
	{
		WAChatRoleInAlliance()
			:header(CHAT_ROLE_IN_ALLIANCE_WA, SERVER_NODE_TYPE_GLOBAL, prot::SS_MSG_TYPE_SYS)
			, role_id(0)
			, alliance_id(0)
		{
		}
	...
	struct WAChatRoleOutAlliance
	{
		WAChatRoleOutAlliance()
			:header(CHAT_ROLE_OUT_ALLIANCE_WA, SERVER_NODE_TYPE_GLOBAL, prot::SS_MSG_TYPE_SYS)
			, role_id(0)
			, alliance_id(0)
		{
		}
	...
	{
		WAChatCampsEnd()
			:header(CHAT_CAMPS_END_WA, SERVER_NODE_TYPE_GLOBAL, prot::SS_MSG_TYPE_SYS)
		{
		}
		prot::SSHeader header;
	};
	...
	{
		WAChatRoleInCamps()
			:header(CHAT_ROLE_IN_CAMPS_WA, SERVER_NODE_TYPE_GLOBAL, prot::SS_MSG_TYPE_SYS)
			, camps_id(0)
			, role_num(0)
		{
