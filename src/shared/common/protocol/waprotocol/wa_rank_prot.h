...
namespace waprot{
	...
	struct WARankRoleAttrChange{
		WARankRoleAttrChange()
			:header(waprot::RANK_ROLE_ATTR_CHANGE_WA, SERVER_NODE_TYPE_GLOBAL, prot::SS_MSG_TYPE_SYS)
			, role_id(0)
			, type(-1)
			, value(0)
			, time(0)
	...
	struct WARankInfo{
		WARankInfo()
			:header(waprot::RANK_INFO_WA, SERVER_NODE_TYPE_GLOBAL, prot::SS_MSG_TYPE_SYS)
			, type(0)
		{}
		prot::SSHeader header;
		Int32 type;			// 排行榜类型
