...
namespace MaNGOS
{
	...
	private:
		...
		//typedef void (*CLOSECALLBACK)(const std::string &sAdd);
		std::function<void(const std::string &sAdd)> m_reconnectHandler;

		void SocketCleanupWork();

	public:
		NetworkThread() : m_work(m_service), m_pendingShutdown(false), m_reconnectHandler(nullptr),
		...
		NetworkThread(std::function<void(const std::string &sAdd)> reconnectHandle) : m_work(m_service), m_pendingShutdown(false), m_reconnectHandler(reconnectHandle),
			m_serviceThread([this] { boost::system::error_code ec; this->m_service.run(ec); }),
			m_socketCleanupThread([this] { this->SocketCleanupWork(); })
		{
			m_serviceThread.detach();
		...
		{
			{
				...
				for (auto i = m_closingSockets.begin(); i != m_closingSockets.end();)//i++
				{
					if (i->get()->Deletable())
					{
						std::string sAdd = i->get()->GetRemoteAddress();
						m_closingSockets.erase(i++);
						if (m_reconnectHandler){
							m_reconnectHandler(sAdd); //自动重连
						}
