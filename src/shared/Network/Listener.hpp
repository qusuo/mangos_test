...
namespace MaNGOS
{
    ...
    class Listener
    {
        ...
        public:
			Listener(int port, int workerThreads, const std::string & listenIP = "0.0.0.0"); //
            ~Listener();
    };

    template <typename SocketType>
	Listener<SocketType>::Listener(int port, int workerThreads, const std::string & listenIP)
		: m_acceptor(m_service, boost::asio::ip::tcp::endpoint(boost::asio::ip::address_v4::from_string(listenIP), port))
    {
    ...
    void Listener<SocketType>::OnAccept(NetworkThread<SocketType> *worker, SocketType *socket, const boost::system::error_code &ec)
    {
		...
		if (ec)
		{
			...
			socket->Close();
			sLog.outString("Connect error:%s", sErr.c_str());
